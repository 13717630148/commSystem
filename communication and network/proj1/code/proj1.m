...
% as "s2" as a relative parameters, note here the s2 is the standard
...
entropy_y = zeros(length(M), length(s2), 1);

for i_M = 1: 1: length(M)
    for i_variation = 1: 1: length(s2)
        entropy_y(i_M, i_variation) = ...
            integral(@(x)entropy_type1(x, M(i_M), s2(i_variation)), ...
            0 - 3 * s2(i_variation), 3 * s2(i_variation) + M(i_M));
        entropy_y(i_M, i_variation) = ...
            entropy_y(i_M, i_variation) - ...
            1 / 2 * log2(2 * pi * exp(1) * s2(i_variation)^2);
        fprintf('i_M = %f, s2 = %f\n', i_M, s2(i_variation))
    end
...
for i_M = 1: 1: length(M)
  semilogx(s2, entropy_y(i_M, :), 'Color', cmap(i_M, :));
  hold on;
  box on;
  grid on;
...
% awgn channel
awgn_entropy = zeros(1, length(s2));

for i_variation = 1: 1: length(s2)
    awgn_entropy(i_variation) = 1 / 2 * log2(1 + 1 / s2(i_variation));
end
legend_name{length(M) + 1} = 'AWGN';
semilogx(s2, awgn_entropy(:), 'Color', cmap(length(M) + 1, :));
legend(legend_name)
